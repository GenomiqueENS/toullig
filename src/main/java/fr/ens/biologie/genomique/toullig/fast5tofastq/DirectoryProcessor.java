...
public class DirectoryProcessor {
     ...
     */
    private static void processSequence(final String sequence, final Writer writer, final LocalReporter localReporter, final String counterName) throws IOException {
        ...
        if (sequence.substring(indexCR1 + 1, indexCR2).equals("")) {
            ...
            localReporter.incrCounter("numberSequenceWrite", counterName + "Write", 1);
        }
    }
     ...
     * @param localReporter local reporter
     */
    private static final void fillCounters(final Fast5 f5, final String status, final LocalReporter localReporter) {
    ...
    private void readFast5WriteFastq(File fast5File, Writer complementWriter,
        ...
        try (Fast5 f5 = new Fast5(fast5File)) {
            ...
            if (complementWriter != null) {
                processSequence(f5.getComplementFastq(), complementWriter, localReporter, status + "_numberSequenceComplement");
            }

            // test if the templateWriter is not null and if the template sequence is
            // not null
            if (templateWriter != null) {
                processSequence(f5.getTemplateFastq(), templateWriter, localReporter, status + "_numberSequenceTemplate");
            }
            ...
            if (consensusWriter != null) {
                processSequence(f5.getConsensusFastq(), consensusWriter, localReporter, status + "_numberSequenceConsensus");
            }
            ...
            if (transcriptWriter != null) {
                processSequence(f5.getTranscriptFastq(), transcriptWriter, localReporter, status + "_numberSequenceTranscript");
            }
     ...
     */
    DirectoryProcessor(final File repertoryFastqOutput, final boolean saveComplementSequence,
                       final boolean saveTemplateSequence,
                       final boolean saveConsensusSequence,
                       final boolean saveTranscriptSequence,
                       final boolean saveCompressGZIP,
                       final boolean saveCompressBZIP2) {
